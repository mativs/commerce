...
function OrderForm() {
  ...
  const [customer, setCustomer] = useState(attempt?.payload.customer || { first_name: '', last_name: '', phone: '', email: '' });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (!attempt && (!/^\d{13,19}$/.test(cardNumber) || !paymentDescription.trim() || !customer.first_name.trim() || !customer.last_name.trim() || !customer.phone.trim() || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(customer.email))) {
        setError('Enter a valid 13–19 digit card number and payment description.');
        setBusy(false); submitting.current = false; return;
      }
      const next = attempt || { key: crypto.randomUUID(), payload: { customer: { ...customer, first_name: customer.first_name.trim(), last_name: customer.last_name.trim(), phone: customer.phone.trim(), email: customer.email.trim().toLowerCase() }, shipping_address: address, notes: notes.trim() || null, credit_card_number: cardNumber, payment_description: paymentDescription.trim(), items: items.map((i) => ({ product_id: i.product_id, quantity: Number(i.quantity) })) } };
    ...
    <form className="checkout-layout" onSubmit={submit}>
      <div>
        ...
        <section className="orders-panel"><h2><span className="step-number">2</span> Customer information</h2><fieldset disabled={locked}>
          <div className="address-grid"><label>First name<input required maxLength={100} autoComplete="given-name" value={customer.first_name} onChange={(e) => setCustomer({ ...customer, first_name: e.target.value })} /></label><label>Last name<input required maxLength={100} autoComplete="family-name" value={customer.last_name} onChange={(e) => setCustomer({ ...customer, last_name: e.target.value })} /></label><label>Phone<input required maxLength={50} autoComplete="tel" type="tel" value={customer.phone} onChange={(e) => setCustomer({ ...customer, phone: e.target.value })} /></label><label>Email<input required maxLength={254} autoComplete="email" type="email" value={customer.email} onChange={(e) => setCustomer({ ...customer, email: e.target.value.toLowerCase() })} /></label></div>
        </fieldset></section>
        <section className="orders-panel"><h2><span className="step-number">3</span> Shipping information</h2><fieldset disabled={locked}>
        ...
        <section className="orders-panel"><h2><span className="step-number">4</span> Payment information</h2><fieldset disabled={locked}>
